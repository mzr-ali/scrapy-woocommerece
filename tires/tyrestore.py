...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('WooCommerece Product Creator')
        ...
        self.category_text = tk.StringVar(self)
    ...
    def __create_widgets(self):
        ...
        ttk.Label(input_frame, text='Enter URL').grid(column=0, row=0, sticky=tk.W)
        # ttk.Label(input_frame, text='Category').grid(column=0, row=1, sticky=tk.W)
        # ttk.Checkbutton(input_frame, text='Single Product', variable=self.is_single).grid(column=1, row=2, sticky=tk.W)
        keyword = ttk.Entry(input_frame, width=30, textvariable=self.keyword_text)
        # category = ttk.Entry(input_frame, width=15, textvariable=self.category_text)
        keyword.focus()
        keyword.grid(column=1, row=0, sticky=tk.W)
        # category.grid(column=1, row=1, sticky=tk.W)
    ...
    def execute_spider(self):
        ...
        if self.feed_text.get() not in self.feed_options:
            ...
            return
        try:
            output_url = f'file:///{self.folder_path_text.get()}/{self.chosen_spider.get()}-{datetime.now().strftime("%d-%m-%H-%M-%S")}.{self.feed_text.get()}'
            ...
            d = r.crawl(self.chosen_spider.get(), keyword=self.keyword_text.get(), category=self.category_text.get())
